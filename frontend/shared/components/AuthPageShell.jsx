...
export function AuthPageShell({ title, subtitle, children }) {
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 overflow-y-auto pt-[calc(env(safe-area-inset-top)+1rem)] pb-[calc(1.5rem+env(safe-area-inset-bottom))] sm:flex sm:flex-col sm:justify-center sm:pt-16 sm:pb-16">
        <main className="relative mx-auto w-full max-w-md px-4 sm:px-6 lg:px-8">
          <div className="mb-5 px-2 text-center sm:mb-6">
            <h1 className="mb-2 select-none text-3xl font-semibold text-slate-800 sm:text-4xl">
              {title}
            </h1>
            <p className="text-slate-600">{subtitle}</p>
          </div>
          <div className="rounded-[2rem] border border-slate-200/80 bg-white/75 p-5 shadow-sm backdrop-blur-xl dark:border-slate-800/60 dark:bg-slate-900/55 sm:rounded-3xl sm:p-8">
            {children}
          </div>
        </main>
      </div>
